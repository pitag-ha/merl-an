...
(** [add_benchmarks ~merlin ~query_time ~current_data samples] appends new
    performance data to [current_data] resulting from running [merlin]
    benchmarks on the [samples] (notice that [samples] also contains info on the
    file and on the query type the samples are for). It returns the updated data
    together with updated [query_time] (the latter is important to analyze the
    performance of this tool itself). *)
